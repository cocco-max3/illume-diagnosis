...
export const questions: Question[] = [
  ...
  },
  {
    id: "steps-too-big",
    key: "steps",
    text: "やることが大きな塊のままで、ひと口サイズまで小さくできていない感じですか？",
    options: [
...
export const resultMap: Record<AnswerKey, DiagnosisResult> = {
  info: {
    ...
    nextAction:
      "今日は、新しい情報を開かずに、今ある候補を3つだけ紙に出してください。その中から「すぐ試せるもの」をひとつ選べたら十分です。",
  ...
  steps: {
    ...
    nextAction:
      "今日は、完成まで考えずに最初の動詞だけ決めてください。開く、書く、置く、送る。そのどれか一つだけで十分です。やってみてください。",
  ...
  emotion: {
    ...
    nextAction:
      "今日は、「誰にも見せない前提」で思考整理の入口だけ作ってください。どれか一つするだけで十分です。やってみてください。",
    nextActionVariants: [
      "今日は、「誰にも見せない前提」で思考整理の入口だけ作ってください。どれか一つするだけで十分です。やってみてください。",
      "今日は、誰かに説明するためではなく、自分のために気持ちをほどくメモを一つ置いてください。どれか一つするだけで十分です。やってみてください。",
      "今日は、形にする前に「何が怖いのか」を一文だけ書いてください。どれか一つするだけで十分です。やってみてください。",
      "今日は、完成させるより先に、頭の中を少しだけ外へ出してください。どれか一つするだけで十分です。やってみてください。",
      "今日は、うまくやる準備ではなく、今の不安に名前をつけてください。どれか一つするだけで十分です。やってみてください。",
      "今日は、誰にも見せないメモとして、思考を置く場所を一つ作ってください。どれか一つするだけで十分です。やってみてください。",
      "今日は、前に進むためではなく、止まっている理由をやさしく見てください。どれか一つするだけで十分です。やってみてください。",
      "今日は、作業を始める前に「戻せる範囲」を一つ決めてください。どれか一つするだけで十分です。やってみてください。",
      "今日は、完成ではなく、気持ちの重さを一段下げる行動を選んでください。どれか一つするだけで十分です。やってみてください。",
      "今日は、怖さを消そうとせず、隣に置いて眺めるだけにしてください。どれか一つするだけで十分です。やってみてください。",
      "今日は、誰にも提出しない前提で、言葉の下書きを一つ置いてください。どれか一つするだけで十分です。やってみてください。",
      "今日は、正しく進むより先に、安心できる小さな入口を探してください。どれか一つするだけで十分です。やってみてください。",
      "今日は、失敗しない方法を探す前に「失敗しても戻れること」を一つ書いてください。どれか一つするだけで十分です。やってみてください。",
      "今日は、気合いを入れずに、今の気持ちを箇条書きで一つだけ出してください。どれか一つするだけで十分です。やってみてください。",
      "今日は、成果を出すより先に、自分を責めないための小さな行動を一つ置いてください。どれか一つするだけで十分です。やってみてください。",
      "今日は、誰かの目線をいったん外して、自分の現在地を一つ書いてください。どれか一つするだけで十分です。やってみてください。",
      "今日は、動けない理由を責めずに、その理由をそのまま一つ書いてください。どれか一つするだけで十分です。やってみてください。",
      "今日は、完成形を考える前に「これなら試せる」を一つ探してください。どれか一つするだけで十分です。やってみてください。",
      "今日は、怖さの奥にある本音を一つだけ拾ってください。どれか一つするだけで十分です。やってみてください。",
      "今日は、前進の証拠ではなく、思考を少し軽くする行動を一つ選んでください。どれか一つするだけで十分です。やってみてください。"
    ],
  ...
  priority: {
    ...
    nextAction:
      "今日は、全部の中から「締切が近い」「すぐ触れる」ものを1つだけ選んでください。選ばなかったものは、今日は持たなくて大丈夫です。",
...
function createInputReflection(problem: string, result: DiagnosisResult): string {
  ...
  if (!cleaned) {
    return "まだうまく言葉になっていなくても大丈夫です。今は、頭の中にあるものを少し外に置こうとしているところです。";
  }

  const clipped = cleaned.length > 42 ? `${cleaned.slice(0, 42)}...` : cleaned;
  const reflections: Record<DiagnosisType, string> = {
    情報過多タイプ: `「${clipped}」と書いてくれた内容を見ると、情報を集めるほど動き出しにくくなっているのかもしれません。今は、増やすより少し減らす方が楽になりそうです。`,
    判断過多タイプ: `「${clipped}」の中には、間違えないように選びたい気持ちが見えます。今は正解を決めきるより、あとで戻れる形にしておく方が合っていそうです。`,
    手順不明タイプ: `「${clipped}」は、やりたいことがないというより、最初の置き場所がまだ見つかっていない状態に近そうです。まずは全体ではなく、最初の一動作だけで大丈夫です。`,
    感情ブレーキタイプ: `「${clipped}」と書いてくれたところに、失敗や反応を先に想像して疲れている感じが少しあります。無理に動くより、安心できる小さな場所を作るところからで大丈夫です。`,
    環境ノイズタイプ: `「${clipped}」を見ていると、集中力そのものより、周りの条件に思考を細かく削られている感じがあります。まずは自分を責めるより、静かな範囲を少し作る方がよさそうです。`,
    完璧主義ロックタイプ: `「${clipped}」には、粗いまま出すことへの抵抗が少し見えます。今は完成度を上げるより、直せる素材として一度外に置く方が進みやすそうです。`,
    優先順位迷子タイプ: `「${clipped}」の中には、いくつものことを同時に持っている重さがありそうです。今は全部を整理するより、今日持つものをひとつに絞る方が合っています。`,
    期待疲労タイプ: `「${clipped}」と書いてくれた内容には、誰かの期待に応えようとして少し疲れている感じがあります。まずは相手の反応より、自分の現在地を確認するところからで大丈夫です。`,
    抱え込み停止タイプ: `「${clipped}」を見ていると、自分だけで何とかしようとして余白がなくなっている感じがあります。全部を渡さなくても、確認だけ外に出すところから始められます。`,
    AI疲れタイプ: `「${clipped}」には、答えや選択肢が増えすぎて、かえって選びにくくなっている感じがあります。今はもっと使いこなすより、受け取る量を小さくする方がよさそうです。`
  };

  return reflections[result.type];
}
